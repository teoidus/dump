function asm_engine(lib, ext, mem) {
'use asm'
...
var ee = 0x00,
  wp = 0x81, wn = 0x41, wb = 0x21, wr = 0x11, wq = 0x09, wk = 0x05,
  bp = 0x82, bn = 0x42, bb = 0x22, br = 0x12, bq = 0x0A, bk = 0x06,
  pawn = 0x20, knight = 0x10, bishop = 0x08, rook = 0x04, queen = 0x02, king = 0x01
//var pieceNames = {0x20: 'p', 0x10: 'n', 0x08: 'b', 0x04: 'r', 0x02: 'q', 0x01: 'k'}
//var pieces = new lib.Uint8Array(mem) // ([wp, wn, wb, wr, wq, wk, bp, bn, bb, br, bq, bk])
var pieces = 0 // ptr to byte[12]
var sliderMask = 0x38, colorMask = 0x3
// bitfields for moves (4 bytes/move): (msb) ep(1) promotion(7) capture(8) ooo(1) to(7) oo(1) from(7)
var promotionMask = 0x7F000000,
      captureMask = 0x00FF0000,
           toMask = 0x00007F00,
         fromMask = 0x0000007F,
           epMask = 0x80000000,
       castleMask = 0x00008080,
           ooMask = 0x00000080,
          oooMask = 0x00008000
//var castleMasks = new lib.Uint8Array(mem) //[ // masks away castle flags during make()
var castleMasks = 12 // ptr to byte[64]
...
//console.log('- board')
//var map0x88 = new lib.Uint8Array(mem)//.map((e, i) => i + (i & ~7))
var map0x88 = 78 // ptr to map0x88
//var board = new lib.Uint8Array(mem)//[
var board = 
...
//  br, bn, bb, bq, bk, bb, bn, br, 0,0,0,0,0,0,0,0
//]
var kings = new lib.Uint8Array(mem)//([sqIDs['e1'], sqIDs['e8']])
var turn = 0 // (0,1) = (w,b)
var moves = 0
var maxLen = 512 // please don't play extra long games
var castling = 0xF // (msb) bq bk wq wk (lsb)
var castlings = new lib.Uint8Array(mem) //(maxLen)
var ep = 0
var eps = new lib.Uint8Array(mem) //(maxLen)
var score = 0

var maxMoveShift = 8 // log2(arbitrary max branching factor)
var moveList = new lib.Uint32Array(mem) // (maxLen * (1 << maxMoveShift))
var moveListMax = new lib.Uint8Array(mem) // (maxLen)

//console.log('- movegen constants')
// piece deltas (used in move generation and attack checking)
var deltas = new lib.Int32Array(mem) //(16 * 256)
//// white pieces
...
var materials = new lib.Int32Array(mem) //(256)
//materials[wp] = 100
//materials[wn] = 320
//materials[wb] = 330
//materials[wr] = 500
...
var evalTable = new lib.Int32Array(mem) // (2 * 256 * 128) // 256 KB
//evalTable.set([
