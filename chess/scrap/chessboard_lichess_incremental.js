...
function print() {
  ...
  console.log('castling:',castling,'ep:',ep,'->',squares[ep],'moves:',moves,'material:',material)
}
...
function make(move) {
  ...
  material -= materials[board[to]] // subtract captured material
  if (((p >> 2)|0) === (king|0))
    kings[turn] = to|0
  ...
  if (((move & castleMask)|0) !== (0|0)) { // handle castling
    ...
    var prom = move & promotionMask
    if (prom !== 0) {// if promotion
      //console.log('is promotion')
      prom >>= 24
      board[to] = prom // place promotion piece
      material -= materials[pawn << 2 | 1 << turn] // subtract friendly pawn material
      material += materials[prom] // add promotion piece material
    } else if ((move & epMask) !== 0) { // else if ep
      //console.log('is ep',epMask,move&epMask,move)
      board[to + sign*16] = 0 // capture ep pawn
      material -= materials[pawn << 2 | 1 << (turn^1)] // subtract enemy pawn material
    }
...
function unmake(move) {
  ...
  material += materials[c] // restore captured material
  if (((p >> 2)|0) === king)
    kings[turn] = fr|0
  ...
  if (((move & castleMask)|0) !== (0|0)) { // handle castling
    ...
    if (prom !== 0) { // if promotion
      prom >>= 24
      var p = pawn << 2 | 1 << turn
      board[fr] = p // place unpromoted pawn
      material += materials[p] // restore friendly pawn material
      material -= materials[prom] // subtract promotion piece material
    } else if (((move & epMask)|0) !== (0|0)) { // else if ep
      board[to + sign*16] = (p ^ 3)|0 // place enemy pawn
      material += materials[pawn << 2 | 1 << (turn^1)] // restore enemy pawn material
    }
  }
}
