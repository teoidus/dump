...
Chessboard.prototype.make = function(move) {
  ...
  if (move.substring(0, 3) == "O-O") {
    ...
    if ((fr == this.sqIDs["a1"]) || (to == this.sqIDs["a1"])) this.castling["wk"] = false;
    if ((fr == this.sqIDs["h1"]) || (to == this.sqIDs["h1"])) this.castling["wq"] = false;
    if ((fr == this.sqIDs["a8"]) || (to == this.sqIDs["a8"])) this.castling["bk"] = false;
    if ((fr == this.sqIDs["h8"]) || (to == this.sqIDs["h8"])) this.castling["bq"] = false;
